  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Executive Dashboard</h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Overview of state meetings, compliance tracking, and department actions.</p>
        </div>
        {canCreateMeeting && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {cardItems.map((c, i) => {
          ...
          return (
            <div
              key={i}
              className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs hover:shadow-md transition-shadow"
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{c.title}</p>
                  <h3 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white mt-1">{c.value}</h3>
                </div>
                <div className={`p-3 sm:p-3.5 rounded-xl ${c.color} shrink-0`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <div>
              <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Monthly Meetings Volume</h3>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400">Total meetings conducted throughout the year</p>
            </div>
            <TrendingUp className="w-5 h-5 text-blue-600 shrink-0" />
          </div>
          <div className="h-56 sm:h-64 w-full min-w-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={charts?.monthlyMeetings || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <div>
              <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Meeting Type Distribution</h3>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400">Breakdown by official meeting category</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Recent System Activities</h3>
            <button onClick={() => navigate('/audit-logs')} className="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline flex items-center space-x-1 cursor-pointer">
              <span>View All</span>
            ...
            {recentActivities?.length === 0 ? (
              ...
              recentActivities?.slice(0, 5).map((log) => (
                <div key={log.id} className="py-3 flex items-start space-x-3">
                  <div className="p-2 bg-slate-100 dark:bg-slate-700 rounded-lg text-slate-600 dark:text-slate-300 mt-0.5 shrink-0">
                    <ShieldAlert className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-slate-900 dark:text-slate-100 line-clamp-2 sm:truncate">{log.details}</p>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Upcoming Action Item Deadlines</h3>
            <button onClick={() => navigate('/actions')} className="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline flex items-center space-x-1 cursor-pointer">
              <span>Tracker</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {upcomingDeadlines?.length === 0 ? (
              ...
              upcomingDeadlines?.map((action) => (
                <div key={action.id} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  <div className="flex items-center justify-between sm:justify-end sm:space-x-3 shrink-0">
