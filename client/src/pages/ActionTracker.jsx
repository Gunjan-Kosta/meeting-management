  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Compliance Action Tracker</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Track department commitments, update progress, and log remarks.</p>
        </div>
      </div>

      {/* Filter Bar */}
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs flex flex-wrap gap-4 items-center">
        <select
          value={statusFilter}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {actions.map((action) => (
            <div
              key={action.id}
              className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-4"
            >
                  ...
                  <div className="flex items-center space-x-2">
                    {action.status === 'PENDING' && (
                      <button
                        ...
                        className="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300 text-xs font-semibold rounded-lg transition-colors"
                      >
                        Start Progress
                      </button>
                    )}
                    <button
                      ...
                      className="px-3.5 py-2 bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold rounded-lg shadow transition-colors"
                    >
                      Mark Completed
                    </button>
                  </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between text-xs">
                <div className="flex items-center space-x-2 text-slate-500 w-full pr-4">
                  ...
                  {editingRemarksId === action.id ? (
                    <div className="flex items-center space-x-2 w-full">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-blue-600 text-white font-semibold rounded-lg text-xs"
                      >
                        Save
                      </button>
                    </div>
