  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Meetings Records</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Search, filter, manage, and monitor state level meetings.</p>
        </div>
        {canCreateMeeting && (
          <button
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-4">
        <form onSubmit={handleSearchSubmit} className="flex flex-col lg:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-700/60 text-slate-900 dark:text-white text-sm rounded-lg border border-slate-200 dark:border-slate-600 focus:ring-2 focus:ring-blue-500 outline-hidden"
            />
          </div>
          <button
            type="submit"
            className="w-full lg:w-auto px-5 py-2.5 bg-slate-900 dark:bg-slate-700 text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-colors"
          >
            Search
          </button>
        </form>
        ...
        {/* Filter Controls Row */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3 pt-2 border-t border-slate-100 dark:border-slate-700/60">
          <select
            value={meetingType}
            onChange={(e) => { setMeetingType(e.target.value); setPage(1); }}
            className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-700/60 text-slate-800 dark:text-slate-200 rounded-lg border border-slate-200 dark:border-slate-600 focus:ring-2 focus:ring-blue-500"
          >
          ...
          <button
            ...
            className="w-full px-3 py-2 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:text-slate-900 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center space-x-1"
          >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  ...
                  <th className="py-3.5 px-4">Total Attendees</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700 text-sm">
                {meetings.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/40 transition-colors">
                    <td className="py-3.5 px-4 font-mono font-semibold text-blue-600 dark:text-blue-400 text-xs">
                      {m.meetingCode}
                    </td>
                    <td className="py-3.5 px-4">
                      <p className="font-semibold text-slate-900 dark:text-white line-clamp-1">{m.title}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{m.meetingType?.replace(/_/g, ' ')}</p>
                    </td>
                    <td className="py-3.5 px-4">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-xs">{m.venue}</p>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-xs font-semibold text-slate-700 dark:text-slate-300">
                      <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-700/80 rounded-full border border-slate-200 dark:border-slate-600">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end space-x-1.5">
                        <button
                          ...
                          title="View Details"
                          className="p-1.5 text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"
                        >
                        ...
                        {canEditMeeting(m.status) && (
                          <button
                            ...
                            title="Edit Meeting"
                            className="p-1.5 text-slate-500 hover:text-amber-600 dark:hover:text-amber-400 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"
                          >
                        ...
                        {canCloseMeeting && m.status === 'SUBMITTED' && (
                          <button
                            ...
                            title="Close Meeting"
                            className="p-1.5 text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"
                          >
          ...
          <div className="px-6 py-4 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg disabled:opacity-40"
              >
